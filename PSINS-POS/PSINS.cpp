...
Date: 17/02/2015, 19/07/2017, 11/12/2018, 30/08/2019, 27/12/2019, 09/04/2020
*/
...
void CFileRdWt::Init(const char *fname0, int columns0)
{
	...
	else if(columns>0)			// txt file read
	{
		...
		while(1)  // skip txt-file comments
		{
			...
			if(feof(f)) break;
			int allSpace=1, allDigital=1;		//判断全空格行和全零行
			for(int i=0; i<sizeof(line); i++)
			{
				char c = line[i];
				if(c=='\n') break;			//换行退出
				if(c!=' ') allSpace = 0;	//如果遇见非空格即不是全空格行
				if( !(c==' '||c==','||c==';'||c==':'||c=='+'||c=='-'||c=='.'||c=='\t'
...
{
	long cur = ftell(f);			//文件位置指针当前位置相对于文件首的偏移字节数
	if(totsize==0)
	{
		fseek(f, 0L, SEEK_END);		//重定位流(数据流/文件)上的文件内部位置指针  将f指针指向文件末尾
		totsize = ftell(f);			//得到文件一共有多少数据量
		fseek(f, cur, SEEK_SET);	//定位当前文件位置
	}
	remsize = totsize-cur;
